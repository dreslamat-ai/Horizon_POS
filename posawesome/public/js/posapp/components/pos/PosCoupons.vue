    ...
    >
      <v-card-title>
        <v-row no-gutters align="center" justify="center">
          <v-col cols="6">
            <span class="text-h6 primary--text">{{ __('الكوبونات') }}</span>
          </v-col>
          <v-col cols="4">
            <v-text-field
              dense
...
export default {
  data: () => ({
    ...
    singleExpand: true,
    items_headers: [
      { title: __('كوبون'), value: 'coupon_code', align: 'start' },
      { title: __('النوع'), value: 'type', align: 'start' },
      { title: __('العرض'), value: 'pos_offer', align: 'start' },
      { title: __('تم تطبيقه'), value: 'applied', align: 'start' },
    ],
  }),
    ...
    add_coupon(new_coupon) {
      ...
      if (!this.customer) {
        evntBus.$emit('show_mesage', {
          text: __('حدّد عميل أولًا قبل تطبيق الكوبون'),
          color: 'error',
        });
        return;
      }
      ...
      if (exist) {
        evntBus.$emit('show_mesage', {
          text: __('تم استخدام الكوبون بالفعل'),
          color: 'error',
        });
        return;
      }
  ...
  created: function () {
    ...
    // كود كوبون مباشر من .cart-summary (بند ١٠ من البروتوتايب) — نفس
    // add_coupon الحقيقية، بدون فتح صفحة الكوبونات الكاملة.
    evntBus.$on('add_coupon_direct', (code) => {
      this.add_coupon(code);
    });
  },
};
</script>
