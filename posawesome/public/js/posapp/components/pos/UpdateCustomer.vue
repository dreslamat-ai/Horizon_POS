...
export default {
  ...
  methods: {
    ...
    clear_customer() {
      ...
      this.referral_code = '';
      this.birthday = '';
      // قيمتان ثابتتان دايمًا لعميل POS — لا تعتمد على
      // frappe.defaults.get_user_default('Customer Group'/'Territory')
      // لأنهما بيرجعوا فاضيين على هذا الموقع، فكان الحفظ يرفض صامتًا
      // (توست "مجموعة العميل مطلوبة" يختفي بسرعة وما يبانش واضح).
      this.group = 'Individual';
      this.territory = 'Egypt';
      this.customer_id = '';
  ...
  created: function () {
    evntBus.$on('open_update_customer', (data) => {
      ...
      if (data && data.prefill_mobile_no) {
        this.mobile_no = data.prefill_mobile_no;
        return;
      }
      if (data) {
    ...
    // قيمتان ثابتتان (راجع clear_customer لنفس السبب) — هذا المسار
    // (created، أول تحميل للمكوّن) هو اللي كان يفلت من الإصلاح
    // الأول: clear_customer بتتنفذ عند إغلاق الديالوج فقط، فأول فتح
    // في الجلسة كان يعتمد على القيم هنا وحدها.
    this.group = 'Individual';
    this.territory = 'Egypt';
  },
};
