        ...
        <v-row class="px-1 py-0" align="start" no-gutters>
          ...
          >
            <v-switch
              ...
              flat
              :label="frappe._('قيمة فرق الديون المعدومة')"
            ></v-switch>
          </v-col>
          <v-col
            cols="6"
          ...
          >
            <v-switch
              ...
              flat
              :label="frappe._('إستخدام رصيد العميل')"
              class="my-0 py-0"
...
export default {
  ...
  methods: {
    ...
    submit(event, payment_received = false, print = false) {
      ...
      if (!payment_received) {
        ...
        if (!phone_payment_is_valid) {
          evntBus.$emit("show_mesage", {
            text: __(
              "الرجاء طلب الدفع عبر الهاتف أو استخدام طريقة دفع أخرى"
            ),
            color: "error",
          });
          frappe.utils.play_sound("error");
          console.error("الدفع عبر الهاتف لم تنجح");
          return;
        }
      }
      ...
      if (
        ...
        evntBus.$emit("show_mesage", {
          text: `برجاء ادخال المبلغ المدفوع`,
          color: "error",
        });
        frappe.utils.play_sound("error");
        return;
      ...
      if (this.paid_change > -this.diff_payment) {
        evntBus.$emit("show_mesage", {
          text: `لا يمكن أن يكون الباقي المدفوع أكبر من الباقي الكلي!`,
          color: "error",
        });
        frappe.utils.play_sound("error");
        return;
      ...
      if (credit_calc_check.length > 0) {
        evntBus.$emit("show_mesage", {
          text: `الرصيد المستبدل لا يمكن ان يكون اكبر من الإجمالي`,
          color: "error",
        });
        frappe.utils.play_sound("error");
        return;
      ...
      if (
        ...
        evntBus.$emit("show_mesage", {
          text: `لا يمكن استبدال رصيد العميل أكثر من إجمالي الفاتورة`,
          color: "error",
        });
        frappe.utils.play_sound("error");
        return;
    ...
    submit_invoice(print) {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            if (print) {
              if (vm.pos_profile.posa_thermal_print_enabled) {
                vm.print_thermal_receipt(vm.invoice_doc.name);
              } else {
                vm.load_print_page();
              }
            }
            ...
            evntBus.$emit("show_mesage", {
              text: `الفاتورة ${r.message.name} تم تسجيلها`,
              color: "success",
            });
    ...
    // QZ Tray thermal receipt printing — rasterizes the receipt server-side
    // (Arabic shaping confirmed correct there, see memory
    // horizon-pos-print-format-fixes-and-raster-decision.md) and sends it to
    // QZ Tray as a raw raster image. QZ Tray must be installed and running
    ...
    print_thermal_receipt(invoice_name) {
      ...
      if (typeof qz === "undefined") {
        evntBus.$emit("show_mesage", {
          text: __("QZ Tray غير مثبّت على هذا الجهاز — لا يمكن الطباعة الحرارية"),
          color: "error",
        });
        return;
      }
      if (!vm.pos_profile.posa_thermal_printer_name) {
        evntBus.$emit("show_mesage", {
          text: __("لم يُحدّد اسم الطابعة الحرارية في إعدادات نقطة البيع"),
          color: "error",
        });
        return;
      }

      const do_print = function () {
        frappe.call({
          method: "posawesome.posawesome.api.thermal_print.get_receipt_raster",
          args: {
          ...
          callback: function (r) {
            if (!r.message || !r.message.image_base64) {
              evntBus.$emit("show_mesage", {
                text: __("تعذّر تجهيز صورة الإيصال للطباعة"),
                color: "error",
              });
              return;
            }
            const config = qz.configs.create(
              vm.pos_profile.posa_thermal_printer_name
            );
            const print_data = [
              {
                type: "raw",
      ...
      qz.security.setCertificatePromise(function (resolve, reject) {
        frappe.call({
          ...
          callback: function (r) {
            r.message ? resolve(r.message) : reject("no certificate");
          },
        });
      });
      ...
      qz.security.setSignaturePromise(function (toSign) {
        return function (resolve, reject) {
          frappe.call({
            ...
            args: { to_sign: toSign },
            callback: function (r) {
              r.message ? resolve(r.message) : reject("signing failed");
            },
          });
        };
      });
      ...
      qz.websocket
        ...
        .catch(function () {
          evntBus.$emit("show_mesage", {
            text: __(
              "تعذّر الاتصال بخدمة QZ Tray على هذا الجهاز — تأكّد من تثبيتها وتشغيلها"
            ),
            color: "error",
          });
        });
    ...
    set_paid_change() {
      ...
      if (this.paid_change > change) {
        this.paid_change_rules = [
          "لا يمكن أن يكون باقي المدفوع أكبر من الباقي الكلي!",
        ];
    ...
    request_payment() {
      ...
      if (!this.invoice_doc.contact_mobile) {
        evntBus.$emit("show_mesage", {
          text: __(`برجاء ادخال رقم موبايل العميل`),
          color: "error",
        });
                ...
                frappe.db
                  ...
                  .then(({ message }) => {
                    if (message.status != "Paid") {
                      ...
                      evntBus.$emit("show_mesage", {
                        text: __(
                          `استغرق طلب الدفع وقتًا طويلاً للرد. يرجى محاولة طلب الدفع مرة أخرى`
                        ),
                        color: "error",
                      });
                    } else {
                      evntBus.$emit("unfreeze");
                      evntBus.$emit("show_mesage", {
                        text: __("تم استلام مبلغ {0} بنجاح.", [
                          vm.formtCurrency(
                            message.grand_total,
  ...
  watch: {
    loyalty_amount(value) {
      if (value > this.available_pioints_amount) {
        ...
        evntBus.$emit("show_mesage", {
          text: `لا يمكن أن يكون مبلغ الولاء أكثر من  ${this.available_pioints_amount}`,
          color: "error",
        });
      } else {
    ...
    redeemed_customer_credit(value) {
      if (value > this.available_customer_credit) {
        evntBus.$emit("show_mesage", {
          text: `يمكنك استرداد رصيد العميل حتى ${this.available_customer_credit}`,
          color: "error",
        });
      }
    },
