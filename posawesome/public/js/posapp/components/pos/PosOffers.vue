<template>
  <div>
    ...
    >
      <v-card-title>
        <span class="text-h6 primary--text">{{ __('العروض') }}</span>
      </v-card-title>
...
export default {
  ...
  data: () => ({
    ...
    singleExpand: true,
    items_headers: [
      { title: __('الإسم'), value: 'name', align: 'start' },
      { title: __('التطبيق على'), value: 'apply_on', align: 'start' },
      { title: __('عرض'), value: 'offer', align: 'start' },
      { title: __('تم تطبيقه'), value: 'offer_applied', align: 'start' },
    ],
  }),
