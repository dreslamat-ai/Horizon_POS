        ...
        >
            ...
            <v-row align="center" no-gutters class="mb-1">
              <v-col md="4" cols="12">
                <v-select
                  ...
                  item-value="name"
                  label="إختار ملف شخصي لنقطة المبيعات"
                ></v-select>
              </v-col>
              <v-col> </v-col>
              <v-col md="3" cols="12">
          ...
          >
            <v-row>
              <v-col md="7" cols="12">
                <p>
                  <strong>{{ __("المدفوعات") }}</strong>
                  <span v-if="total_unallocated_amount" class="primary--text">
                    {{ __("- إجمالي المبلغ الغير مخصص") }} :
                    {{ currencySymbol(pos_profile.currency) }}
          ...
          <div v-if="pos_profile.posa_allow_mpesa_reconcile_payments">
            <v-row>
              <v-col md="8" cols="12">
                <p>
                  <span
                    ><strong>{{ __("البحث في المدفوعات") }}</strong></span
                  >
                </p>
              </v-col>
            ...
            <v-row align="center" no-gutters class="mb-1">
              ...
              <v-col md="4" cols="12" class="mr-1">
                <v-text-field
                  ...
                  outlined
                  color="primary"
                  :label="frappe._('البحث برقم الموبايل')"
                  background-color="white"
                  hide-details
        ...
        >
          <strong>
            ...
            <v-row v-if="total_selected_payments">
              <v-col md="7" class="mt-1"
                ><span>{{ __("إجمالي المدفوعات") }}</span></v-col
              >
              <v-col md="5">
                <v-text-field
                  class="p-0 m-0"
            ...
            <v-row v-if="total_selected_mpesa_payments">
              <v-col md="7" class="mt-1"
                ><span>{{ __("إجمالي MPESA :") }}</span></v-col
              >
              <v-col md="5">
                <v-text-field
                  class="p-0 m-0"
            ...
            <v-row>
              <v-col md="7">
                <h4 class="primary--text mt-1">{{ __("الفرق :") }}</h4>
              </v-col>
              <v-col md="5">
                <v-text-field
                  class="p-0 m-0"
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      const vm = this;
      if (!customer) {
        frappe.throw(__("برجاء اختيار عميل"));
        return;
      }
      if (
        ...
        frappe.throw(__("برجاء الدفع او اختيار دفعة"));
        return;
      }
      if (
        this.total_selected_payments > 0 &&
        ...
        frappe.throw(__("برجاء اختيار فاتورة"));
        return;
      }
      ...
      frappe.call({
        ...
        args: { payload },
        freeze: true,
        freeze_message: __("معالجة الدفع"),
        callback: function (r) {
          if (r.message) {
