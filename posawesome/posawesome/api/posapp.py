...
# set_batch_nos اتشالت من erpnext v16 (إعادة هيكلة Serial and Batch
# Bundle) — الاستيراد المباشر كان بيمنع تحميل الموديول كله بـImportError
# صامت حتى لو الميزة (posa_auto_set_batch) مش مفعّلة أصلاً. الاستخدام
# الوحيد محاط الآن برسالة واضحة بدل انهيار الصفحة بالكامل.
...
def get_item_group_condition(pos_profile):
    # كان بيستبدل %s بايثونيًّا خامًا بدل تمرير قيم SQL آمنة — أي اسم
    # مجموعة فيه نص عربي أو مسافة كان بيدخل الاستعلام بلا علامات
    # اقتباس ويُفسَّر كاسم عمود لا قيمة، فيرمي OperationalError صريحًا
    # (اتقاس فعليًّا على مجموعة "سترة" في sutra.horizonerp.cloud).
    cond = " and 1=1"
    item_groups = get_item_groups(pos_profile)
    if item_groups:
        cond = " and item_group in (%s)" % (
            ", ".join([frappe.db.escape(d) for d in item_groups])
        )
...
def submit_invoice(invoice, data):
    ...
    if frappe.get_value("POS Profile", invoice_doc.pos_profile, "posa_auto_set_batch"):
        frappe.throw(
            _(
                "تحديد الباتش التلقائي (posa_auto_set_batch) غير مدعوم على"
                " هذا الإصدار من erpnext — النظام الجديد يعتمد Serial and"
                " Batch Bundle بدل batch_no المباشر. عطّل الخيار من POS"
                " Profile لحد ما تُبنى ميزة بديلة متوافقة."
            )
        )
...
def get_product_bundles():
    # بند ٩: كل الحزم المعرَّفة فعليًا مع مكوّناتها وسعر البيع —
    # الاكتشاف نفسه (مطابقة محتوى السلة) بيحصل في الواجهة، هنا بس
    # قراءة البيانات الحقيقية من Product Bundle + Item Price.
    bundles = frappe.get_list("Product Bundle", fields=["name", "new_item_code"])
    result = []
    for b in bundles:
        ...
        item_name = frappe.db.get_value("Item", b.new_item_code, "item_name")
        rate = frappe.db.get_value(
            "Item Price",
            {"item_code": b.new_item_code, "price_list": "Standard Selling"},
            "price_list_rate",
        )
        result.append({
            "bundle_item_code": b.new_item_code,
...
@frappe.whitelist()
def verify_manager_pin(pos_profile, pin):
    # التحقق من الـPIN من جانب السيرفر لا العميل — نفس القيمة
    # المخزّنة في POS Profile بتُقرأ هنا مباشرة من قاعدة البيانات،
    # فلا يمكن تجاوزها من console المتصفح (بند ٦ من بروتوتايب
    # كاشير سترة). القيمة الحقيقية لا تصل للواجهة إطلاقًا.
    stored_pin = frappe.db.get_value("POS Profile", pos_profile, "posa_manager_pin")
    if not stored_pin:
        return {"valid": False}
    return {"valid": str(pin) == str(stored_pin)}
